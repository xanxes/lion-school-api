...
//Todos os alunos de cada curso
const getAlunos = (courseName) => {
    let studentsList = [];
    let error = true;
...
//Alunos de acordo com o status
    const getStatusAluno = (studentStatus) => {
...
//Listar as disciplinas atraves do numero de matricula
const getDisciplinaAluno = function(matricula){
        ...
        let erro = true
            ...
            if (typeof(numeroMatricula) != `undefined`) {
                if (numeroMatricula != "") {
               
                    alunos.forEach (item => {
                        item.curso.forEach (item2 => {
                            if (item.matricula == numeroMatricula) {
                            infoAluno.nome = item.nome
                            infoAluno.foto = item.foto    
            ...
            if (erro) {
                ...
                infoAluno.disciplinas = mats
                return infoAluno
            }
        }

const getAluno = (studentEnrollment) => {
            let student = {};
            let erro = true;
        ...
        const getStatusAlunoFiltro = (studentsArray, studentStatus) => {
            let status = studentStatus.toLowerCase();
        ...
        const getAlunoPorAnoFiltro = (studentsArray, year) => {
